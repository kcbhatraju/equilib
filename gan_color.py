...
RGB = transforms.ToTensor()(Image.open("equi.png"))

BGR = np.array(Image.open("equi.png"))
BGR = BGR[:,:,::-1].copy()
BGR = transforms.ToTensor()(BGR)
...
for i in range(img_sets):
    ...
    for j in range(batch_size):
        fov = torch.normal(100.,0.02,(1,)).requires_grad_()
        ...
        new_img = lib(equi=RGB, rots=rots)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.fov = torch.normal(100.,0.02,(1,)).requires_grad_()
        self.roll = torch.normal(0.,0.02,(1,)).requires_grad_()
        self.pitch = torch.normal(0.,0.02,(1,)).requires_grad_()
        
        self.conv1 = nn.Conv2d(3,64,kernel_size=(1,1))
        self.conv2 = nn.Conv2d(64,128,kernel_size=(1,1))
        self.conv3 = nn.Conv2d(128,3,kernel_size=(1,1))
    ...
    def forward(self, yaw):
        ...
        for i in range(batch_size):
            ...
            new_img = lib(equi=BGR, rots=rots)
            new_img = NORM(new_img)
        ...
        # torchviz.make_dot(out).render("ztest_img",format="png")
        return out
...
for e in range(num_epochs):
    ...
    for i in range(img_sets):
        ...
        for j in range(gen_takes):
            ...
            """with torch.no_grad():
                netG.fov.data = torch.clamp(netG.fov.data-5.5*netG.fov.grad,min=0,max=100)
                netG.roll.data = torch.clamp(netG.roll.data-6*netG.roll.grad,min=0,max=2*PI)
                netG.pitch.data = torch.clamp(netG.pitch.data-6*netG.pitch.grad,min=0,max=2*PI)"""

            optimG.step()
            print("Testing Conv1 Weight Grad: ", netG.conv1.weight.grad)
