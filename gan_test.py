...
for i in range(img_sets):
    ...
    for j in range(batch_size):
        fov = torch.normal(40.,0.02,(1,)).requires_grad_()
        roll = torch.normal(PI/2,0.02,(1,)).requires_grad_()
        pitch = torch.normal(PI,0.02,(1,)).requires_grad_()
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.fov = torch.normal(90.,0.02,(1,)).requires_grad_()
...
for e in range(num_epochs):
    ...
    for i in range(img_sets):
        ...
        for j in range(gen_takes):
            ...
            with torch.no_grad():
                netG.fov.data = torch.clamp(netG.fov.data-0.5*netG.fov.grad,min=0,max=100)
                netG.roll.data = torch.clamp(netG.roll.data-0.5*netG.roll.grad,min=0,max=2*PI)
                netG.pitch.data = torch.clamp(netG.pitch.data-0.5*netG.pitch.grad,min=0,max=2*PI)
